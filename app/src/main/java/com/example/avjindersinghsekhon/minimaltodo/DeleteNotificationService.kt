...
class DeleteNotificationService : IntentService("DeleteNotificationService") {
    ...
    override fun onHandleIntent(intent: Intent?) {
        ...
        mToDoItems = loadData()
        mItem = mToDoItems!!.filter { item -> item?.identifier == todoID }.firstOrNull()
        mItem.let {
    ...
    private fun dataChanged() {
        baseContext.getSharedPreferences(MainActivity.SHARED_PREF_DATA_SET_CHANGED, Context.MODE_PRIVATE)
                .edit().run { putBoolean(MainActivity.CHANGE_OCCURED, true) }.apply()
    }
